...
import { constructAlias, constructDefaultAlias } from './identifiers.utils';
 ...
 */
export async function createDefaultIdentifier(): Promise<IdentifierDto> {
  const agent = getAgent();
  const { identifiersRepository } = getRequestContext();
  const alias = constructDefaultAlias();
  ...
  return identifiersRepository.findIdentifier({ did: identifier.did! }).then((identifier) => {
    if (!identifier) throw new Error('Identifier not found');
    return toIdentifierDto(identifier);
  });
}
